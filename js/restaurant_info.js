  ...
  fetchRestaurantFromURL((error, restaurant) => {
    if (error) { // Got an error!
      ...
      L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.jpg70?access_token={mapboxToken}', {
        mapboxToken: 'pk.eyJ1IjoiYW1pbmRlIiwiYSI6ImNqbXNiNDNyeDAxZHkzb3FrbnZmb3NyaHEifQ.FmjR2asCAlbI-LW7uwHutg',
        maxZoom: 18,
...
fillRestaurantHTML = (restaurant = self.restaurant) => {
  ...
  var aria_name = document.getElementById('name'); //holds restaurant name
  aria_name.innerHTML = "Restaurant name: " + restaurant.name;
  const address = document.getElementById('restaurant-address');
  address.innerHTML = restaurant.address;
  
  var aria_address = document.getElementById('address'); //holds restaurant address
  ...
  var aria_cuisine = document.getElementById('cuisine'); //holds restaurant cuisine
...
fillRestaurantHoursHTML = (operatingHours = self.restaurant.operating_hours) => {
  ...
  for (let key in operatingHours) {
	...
	//adding tabindex=0 to each of operating hours rows so the screen reader can recognize them individualy and read the content
...
createReviewHTML = (review) => {
  ...
  li.setAttribute('tabindex' , "0") ;//adding tabindex to review list																	   
